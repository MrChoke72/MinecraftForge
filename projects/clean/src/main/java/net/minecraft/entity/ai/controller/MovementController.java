...
public class MovementController {
   ...
   //AH CHANGE **** tick() method override
   public void tick() {
      if ((mob instanceof CreatureEntity) && (!(((CreatureEntity)mob).getNavigator() instanceof ClimberPathNavigator)))
      {
         tickChoke();
      }
      else
   ...
   private void tickVanilla() {
      if (this.action == MovementController.Action.STRAFE) {
         ...
         this.action = MovementController.Action.WAIT;
         double d0 = this.posX - this.mob.getPosX();
         double d1 = this.posZ - this.mob.getPosY();
         double d2 = this.posY - this.mob.getPosZ();
         double d3 = d0 * d0 + d2 * d2 + d1 * d1;
         if (d3 < (double)2.5000003E-7F) {
            this.mob.setMoveForward(0.0F);
            return;
