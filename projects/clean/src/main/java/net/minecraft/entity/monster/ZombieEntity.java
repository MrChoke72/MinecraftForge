...
public class ZombieEntity extends MonsterEntity {
      ...
      if (this.canBreakDoors()) {
         if (this.isBreakDoorsTaskSet != enabled) {
            ...
            if (enabled) {
               ...
               //AH CHANGE ADD
               this.goalSelector.addGoal(1, this.openTrapDoor);

            } else {
               this.goalSelector.removeGoal(this.breakDoor);

               //AH CHANGE ADD
               this.goalSelector.removeGoal(this.openTrapDoor);
            }
         }
   ...
   protected void onDrowned() {
      this.convert(EntityType.DROWNED);
   ...
   public ILivingEntityData onInitialSpawn(IWorld worldIn, DifficultyInstance difficultyIn, SpawnReason reason, @Nullable ILivingEntityData spawnDataIn, @Nullable CompoundNBT dataTag) {
      ...
      if (spawnDataIn == null) {
         ...
         //spawnDataIn = new ZombieEntity.GroupData(worldIn.getRandom().nextFloat() <= 1.0F);  //This is the chance to be a baby
         spawnDataIn = new ZombieEntity.GroupData(worldIn.getRandom().nextFloat() < 0.05F);  //This is the chance to be a baby
      }
      ...
      if (spawnDataIn instanceof ZombieEntity.GroupData) {
         ...
         if (zombieentity$groupdata.isChild) {
            ...
            //AH CHANGE CANCEL TEMP FOR TESTING
            //} else if ((double)worldIn.getRandom().nextFloat() <= 1.0D) {
