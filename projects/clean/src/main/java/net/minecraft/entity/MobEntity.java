...
public abstract class MobEntity extends LivingEntity {
   ...
   //AH CHANGE START ******
   public void setMoveLadder(float moveLadder) {
      this.moveLadder = moveLadder;
   }

   @Override
   public boolean isOnLadder() {
      return this.getLadderBlockState(0) != null;
   }

   public boolean isOnScaffolding() {
      int i = MathHelper.floor(this.getPosX());
      int j = MathHelper.floor(this.getBoundingBox().minY + -1);
   ...
   @Nullable
   public BlockState getLadderBlockState(int yOffSet) {
      int i = MathHelper.floor(this.getPosX());
      int j = MathHelper.floor(this.getBoundingBox().minY + yOffSet);
      int k = MathHelper.floor(this.getPosZ());

      BlockPos pos = new BlockPos(i, j, k);
      BlockState state = this.world.getBlockState(pos);
      Block block = state.getBlock();
      if (BlockTags.WOODEN_TRAPDOORS.contains(block)) {
         return state;
      } else {
         Material material = state.getMaterial();
         if (material == Material.WATER || material == Material.LAVA) {
            pos = new BlockPos(i, j + 1, k);
            state = this.world.getBlockState(pos);
         }

         return state.getBlock().isLadder(state, this.world, pos) ? state : null;
      }
   }
   //AH CHANGE END *****
}
