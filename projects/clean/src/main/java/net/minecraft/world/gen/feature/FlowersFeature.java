...
public abstract class FlowersFeature<U extends IFeatureConfig> extends Feature<U> {
   ...
   public boolean place(IWorld worldIn, ChunkGenerator<? extends GenerationSettings> generator, Random rand, BlockPos pos, U config) {
      BlockState blockstate = this.func_225562_b_(rand, pos, config);
      int i = 0;

      for(int j = 0; j < this.func_225560_a_(config); ++j) {
         BlockPos blockpos = this.func_225561_a_(rand, pos, config);
         if (worldIn.isAirBlock(blockpos) && blockpos.getY() < 255 && blockstate.isValidPosition(worldIn, blockpos) && this.func_225559_a_(worldIn, blockpos, config)) {
