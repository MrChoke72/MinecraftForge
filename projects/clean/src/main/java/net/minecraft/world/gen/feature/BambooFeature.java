      ...
      if (worldIn.isAirBlock(blockpos$mutable)) {
         if (Blocks.BAMBOO.getDefaultState().isValidPosition(worldIn, blockpos$mutable)) {
               ...
               for(int l = pos.getX() - k; l <= pos.getX() + k; ++l) {
                  for(int i1 = pos.getZ() - k; i1 <= pos.getZ() + k; ++i1) {
                     ...
                     if (j1 * j1 + k1 * k1 <= k * k) {
                        ...
                        if (func_227250_b_(worldIn.getBlockState(blockpos$mutable1).getBlock())) {
            ...
            for(int l1 = 0; l1 < j && worldIn.isAirBlock(blockpos$mutable); ++l1) {
               worldIn.setBlockState(blockpos$mutable, field_214566_a, 2);
               blockpos$mutable.move(Direction.UP, 1);
            }

            if (blockpos$mutable.getY() - pos.getY() >= 3) {
               worldIn.setBlockState(blockpos$mutable, field_214567_aS, 2);
               worldIn.setBlockState(blockpos$mutable.move(Direction.DOWN, 1), field_214568_aT, 2);
               worldIn.setBlockState(blockpos$mutable.move(Direction.DOWN, 1), field_214569_aU, 2);
            }
         }
