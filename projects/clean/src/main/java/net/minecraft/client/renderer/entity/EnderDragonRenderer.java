...
public class EnderDragonRenderer extends EntityRenderer<EnderDragonEntity> {
   ...
   private static final RenderType field_229053_h_ = RenderType.entityCutoutNoCull(DRAGON_TEXTURES);
   private static final RenderType field_229054_i_ = RenderType.entityDecal(DRAGON_TEXTURES);
   private static final RenderType field_229055_j_ = RenderType.eyes(field_229052_g_);
   private static final RenderType field_229056_k_ = RenderType.entitySmoothCutout(ENDERCRYSTAL_BEAM_TEXTURES);
   ...
   public void render(EnderDragonEntity entityIn, float entityYaw, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn) {
      ...
      if (entityIn.deathTicks > 0) {
         ...
         IVertexBuilder ivertexbuilder = bufferIn.getBuffer(RenderType.entityAlpha(DRAGON_EXPLODING_TEXTURES, f2));
         this.field_229058_m_.render(matrixStackIn, ivertexbuilder, packedLightIn, OverlayTexture.DEFAULT_LIGHT, 1.0F, 1.0F, 1.0F, 1.0F);
      ...
      this.field_229058_m_.render(matrixStackIn, ivertexbuilder4, packedLightIn, OverlayTexture.DEFAULT_LIGHT, 1.0F, 1.0F, 1.0F, 1.0F);
      if (entityIn.deathTicks > 0) {
   ...
   public static void func_229059_a_(float p_229059_0_, float p_229059_1_, float p_229059_2_, float p_229059_3_, int p_229059_4_, MatrixStack p_229059_5_, IRenderTypeBuffer p_229059_6_, int p_229059_7_) {
      ...
      for(int j = 1; j <= 8; ++j) {
         ...
         ivertexbuilder.pos(matrix4f, f4 * 0.2F, f5 * 0.2F, 0.0F).color(0, 0, 0, 255).tex(f6, f2).overlay(OverlayTexture.DEFAULT_LIGHT).lightmap(p_229059_7_).normal(matrix3f, 0.0F, -1.0F, 0.0F).endVertex();
         ivertexbuilder.pos(matrix4f, f4, f5, f1).color(255, 255, 255, 255).tex(f6, f3).overlay(OverlayTexture.DEFAULT_LIGHT).lightmap(p_229059_7_).normal(matrix3f, 0.0F, -1.0F, 0.0F).endVertex();
         ivertexbuilder.pos(matrix4f, f7, f8, f1).color(255, 255, 255, 255).tex(f9, f3).overlay(OverlayTexture.DEFAULT_LIGHT).lightmap(p_229059_7_).normal(matrix3f, 0.0F, -1.0F, 0.0F).endVertex();
         ivertexbuilder.pos(matrix4f, f7 * 0.2F, f8 * 0.2F, 0.0F).color(0, 0, 0, 255).tex(f9, f2).overlay(OverlayTexture.DEFAULT_LIGHT).lightmap(p_229059_7_).normal(matrix3f, 0.0F, -1.0F, 0.0F).endVertex();
         f4 = f7;
         f5 = f8;
         f6 = f9;
      }
   ...
   public static class EnderDragonModel extends EntityModel<EnderDragonEntity> {
      ...
      public void render(EnderDragonEntity entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
      }
