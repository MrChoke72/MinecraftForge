...
public class SpriteAwareVertexBuilder implements IVertexBuilder {
   ...
   public void vertex(float x, float y, float z, float red, float green, float blue, float alpha, float texU, float texV, int overlayUV, int lightmapUV, float normalX, float normalY, float normalZ) {
      this.vertexBuilder.vertex(x, y, z, red, green, blue, alpha, this.atlasSprite.getInterpolatedU((double)(texU * 16.0F)), this.atlasSprite.getInterpolatedV((double)(texV * 16.0F)), overlayUV, lightmapUV, normalX, normalY, normalZ);
   }
}
