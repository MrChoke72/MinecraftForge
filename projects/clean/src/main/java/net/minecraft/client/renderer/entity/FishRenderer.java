   ...
   public void func_225623_a_(FishingBobberEntity p_225623_1_, float p_225623_2_, float p_225623_3_, MatrixStack p_225623_4_, IRenderTypeBuffer p_225623_5_, int p_225623_6_) {
      ...
      if (playerentity != null) {
         ...
         if ((this.renderManager.options == null || this.renderManager.options.thirdPersonView <= 0) && playerentity == Minecraft.getInstance().player) {
            ...
            d4 = MathHelper.lerp((double)p_225623_3_, playerentity.prevPosX, playerentity.getPosX()) + vec3d.x;
            d5 = MathHelper.lerp((double)p_225623_3_, playerentity.prevPosY, playerentity.getPosY()) + vec3d.y;
            d6 = MathHelper.lerp((double)p_225623_3_, playerentity.prevPosZ, playerentity.getPosZ()) + vec3d.z;
            f3 = playerentity.getEyeHeight();
         } else {
            d4 = MathHelper.lerp((double)p_225623_3_, playerentity.prevPosX, playerentity.getPosX()) - d1 * d2 - d0 * 0.8D;
            d5 = playerentity.prevPosY + (double)playerentity.getEyeHeight() + (playerentity.getPosY() - playerentity.prevPosY) * (double)p_225623_3_ - 0.45D;
            d6 = MathHelper.lerp((double)p_225623_3_, playerentity.prevPosZ, playerentity.getPosZ()) - d0 * d2 + d1 * 0.8D;
            f3 = playerentity.isCrouching() ? -0.1875F : 0.0F;
         }

         double d9 = MathHelper.lerp((double)p_225623_3_, p_225623_1_.prevPosX, p_225623_1_.getPosX());
         double d10 = MathHelper.lerp((double)p_225623_3_, p_225623_1_.prevPosY, p_225623_1_.getPosY()) + 0.25D;
