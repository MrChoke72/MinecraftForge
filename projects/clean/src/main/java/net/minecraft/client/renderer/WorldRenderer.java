...
public class WorldRenderer implements AutoCloseable, IResourceManagerReloadListener {
   ...
   public void getViewVector(MatrixStack matrixStackIn, float partialTicks, long finishTimeNano, boolean drawBlockOutline, ActiveRenderInfo activeRenderInfoIn, GameRenderer gameRendererIn, LightTexture lightmapIn, Matrix4f projectionIn) {
      ...
      this.renderBlockLayer(RenderType.solid(), matrixStackIn, d0, d1, d2);
      this.renderBlockLayer(RenderType.cutoutMipped(), matrixStackIn, d0, d1, d2);
      this.renderBlockLayer(RenderType.cutout(), matrixStackIn, d0, d1, d2);
      RenderHelper.setupLevelDiffuseLighting(matrixStackIn.getLast().getPositionMatrix());
      ...
      irendertypebuffer$impl.finish(RenderType.entitySolid(AtlasTexture.LOCATION_BLOCKS_TEXTURE));
      irendertypebuffer$impl.finish(RenderType.entityCutout(AtlasTexture.LOCATION_BLOCKS_TEXTURE));
      irendertypebuffer$impl.finish(RenderType.entityCutoutNoCull(AtlasTexture.LOCATION_BLOCKS_TEXTURE));
      irendertypebuffer$impl.finish(RenderType.entitySmoothCutout(AtlasTexture.LOCATION_BLOCKS_TEXTURE));
               ...
               if (sortedset != null && !sortedset.isEmpty()) {
                  ...
                  if (k3 >= 0) {
                     ...
                     irendertypebuffer1 = (p_230014_2_) -> {
                        ...
                        return p_230014_2_.getUseDelegate() ? VertexBuilderUtils.newDelegate(ivertexbuilder, ivertexbuilder3) : ivertexbuilder3;
                     };
                  }
               }
      ...
      this.checkMatrixStack(matrixStackIn);
      irendertypebuffer$impl.finish(RenderType.solid());
      ...
      irendertypebuffer$impl.finish(RenderType.glint());
      irendertypebuffer$impl.finish(RenderType.entityGlint());
      irendertypebuffer$impl.finish(RenderType.waterMask());
      this.renderTypeTextures.getCrumblingBufferSource().finish();
      irendertypebuffer$impl.finish(RenderType.lines());
      irendertypebuffer$impl.finish();
      iprofiler.endStartSection("translucent");
      this.renderBlockLayer(RenderType.translucent(), matrixStackIn, d0, d1, d2);
   ...
   private void renderBlockLayer(RenderType blockLayerIn, MatrixStack matrixStackIn, double xIn, double yIn, double zIn) {
      ...
      boolean flag = blockLayerIn != RenderType.translucent();
      ...
      while(true) {
         ...
         if (!chunkrenderdispatcher$chunkrender.getCompiledChunk().isLayerEmpty(blockLayerIn)) {
            ...
            vertexbuffer.draw(matrixStackIn.getLast().getPositionMatrix(), 7);
            matrixStackIn.pop();
         }
      }
   ...
   public void renderSky(MatrixStack matrixStackIn, float partialTicks) {
      if (this.mc.world.dimension.getType() == DimensionType.THE_END) {
         ...
         this.skyVBO.draw(matrixStackIn.getLast().getPositionMatrix(), 7);
         VertexBuffer.unbindBuffer();
         ...
         Matrix4f matrix4f1 = matrixStackIn.getLast().getPositionMatrix();
         float f12 = 30.0F;
         ...
         if (f10 > 0.0F) {
            ...
            this.vertexBufferFormat.setupBufferState(0L);
            this.starVBO.draw(matrixStackIn.getLast().getPositionMatrix(), 7);
            VertexBuffer.unbindBuffer();
            this.vertexBufferFormat.clearBufferState();
         }
         ...
         if (d0 < 0.0D) {
            ...
            this.vertexBufferFormat.setupBufferState(0L);
            this.sky2VBO.draw(matrixStackIn.getLast().getPositionMatrix(), 7);
            VertexBuffer.unbindBuffer();
            this.vertexBufferFormat.clearBufferState();
            matrixStackIn.pop();
         }
      ...
      if (this.mc.world.dimension.isSurfaceWorld()) {
         ...
         if (this.cloudsVBO != null) {
            ...
            for(int l = i1; l < 2; ++l) {
               ...
               this.cloudsVBO.draw(matrixStackIn.getLast().getPositionMatrix(), 7);
            }
