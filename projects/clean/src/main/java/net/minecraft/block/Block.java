...
public class Block implements IItemProvider {
   ...
   public static Direction getDirFacingFromState(BlockState blockState) {
      Direction dirFacing = null;

      boolean b = blockState.get(SixWayBlock.NORTH);
      if (b) {
         dirFacing = Direction.NORTH;
      } else {
         b = blockState.get(SixWayBlock.SOUTH);
         if (b) {
            dirFacing = Direction.SOUTH;
         } else {
            b = blockState.get(SixWayBlock.EAST);
            if (b) {
               dirFacing = Direction.EAST;
            } else {
               b = blockState.get(SixWayBlock.WEST);
               if (b) {
